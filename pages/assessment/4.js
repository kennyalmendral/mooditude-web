...
export default function Assessment4() {
  ...
  useEffect(() => {
    if (localStorage.getItem('currentAssessmentStep') !== null) {
      localStorage.setItem('currentAssessmentStep', 4)

      console.log(`Current assessment step: ${localStorage.getItem('currentAssessmentStep')}`)
    }

    if (localStorage.getItem('assessmentStep4Answer') > 0) {
      setAssessmentStep4Answer(localStorage.getItem('assessmentStep4Answer'))
    }
  ...
  const handleChange = (e) => {
    clearInterval(timer)
    localStorage.setItem('assessmentStep4Time', assessmentStep4Time)
    console.log(`Timer cleared at ${assessmentStep4Time} second(s)`)
  ...
  const handleNextStep = () => {
    ...
    if (assessmentStep4Answer !== '') {
      localStorage.setItem('assessmentStep4Answer', parseInt(assessmentStep4Answer))
      ...
      <div className={`${styles.onboarding_wrapper} ${styles.on_assessment_wrapper}`}>
        <div className={styles.onboarding_inner_wrapper}>
          ...
          <div className={`custom_stepper_wrap ${styles.custom_stepper_wrapper}`}>
            <Stepper activeStep={3} alternativeLabel={true} epand="true">
              {steps.map((label) => (
                <Step key={label}>
            ...
            <FormControl component="fieldset" error={formError} onChange={() => {setFormError(false)}}>
              ...
              <RadioGroup>
                <FormControlLabel 
                  ...
                  control={<Radio icon={<RadioButtonUncheckedRoundedIcon />} checkedIcon={<CheckCircleRoundedIcon  />} sx={{'&.Mui-checked': {color: '#F8E71C'}}} checked={assessmentStep4Answer == 1} onChange={handleChange} />} 
                ...
                <FormControlLabel value="2" control={<Radio icon={<RadioButtonUncheckedRoundedIcon />} checkedIcon={<CheckCircleRoundedIcon  />} sx={{'&.Mui-checked': {color: '#F8E71C'}}} checked={assessmentStep4Answer == 2} onChange={handleChange} />} label="Rarely" />
                <FormControlLabel value="3" control={<Radio icon={<RadioButtonUncheckedRoundedIcon />} checkedIcon={<CheckCircleRoundedIcon  />} sx={{'&.Mui-checked': {color: '#F8E71C'}}} checked={assessmentStep4Answer == 3} onChange={handleChange} />} label="Sometimes" />
                <FormControlLabel value="4" control={<Radio icon={<RadioButtonUncheckedRoundedIcon />} checkedIcon={<CheckCircleRoundedIcon  />} sx={{'&.Mui-checked': {color: '#F8E71C'}}} checked={assessmentStep4Answer == 4} onChange={handleChange} />} label="Often" />
                <FormControlLabel value="5" control={<Radio icon={<RadioButtonUncheckedRoundedIcon />} checkedIcon={<CheckCircleRoundedIcon  />} sx={{'&.Mui-checked': {color: '#F8E71C'}}} checked={assessmentStep4Answer == 5} onChange={handleChange} />} label="Most of the time" />
              </RadioGroup>
              {
                formError ? 
