...
export default function Assessment19() {
  ...
  useEffect(() => {
    if (localStorage.getItem('currentAssessmentStep') !== null) {
      localStorage.setItem('currentAssessmentStep', 19)

      console.log(`Current assessment step: ${localStorage.getItem('currentAssessmentStep')}`)
    }

    if (localStorage.getItem('assessmentStep19Answer') > 0) {
      setAssessmentStep19Answer(localStorage.getItem('assessmentStep19Answer'))
    }
  ...
  const handleChange = (e) => {
    clearInterval(timer)
    localStorage.setItem('assessmentStep19Time', assessmentStep19Time)
    console.log(`Timer cleared at ${assessmentStep19Time} second(s)`)
  ...
  const handleNextStep = () => {
    ...
    if (assessmentStep19Answer !== '') {
      localStorage.setItem('assessmentStep19Answer', parseInt(assessmentStep19Answer))
            ...
            <FormControl component="fieldset" error={formError} onChange={() => {setFormError(false)}}>
              ...
              <RadioGroup>
                <FormControlLabel 
                  ...
                  control={<Radio icon={<RadioButtonUncheckedRoundedIcon />} checkedIcon={<CheckCircleRoundedIcon  />} sx={{'&.Mui-checked': {color: '#F8E71C'}}} checked={assessmentStep19Answer == 1} onChange={handleChange} />} 
                ...
                <FormControlLabel value="2" control={<Radio icon={<RadioButtonUncheckedRoundedIcon />} checkedIcon={<CheckCircleRoundedIcon  />} sx={{'&.Mui-checked': {color: '#F8E71C'}}} checked={assessmentStep19Answer == 2} onChange={handleChange} />} label="Rarely" />
                <FormControlLabel value="3" control={<Radio icon={<RadioButtonUncheckedRoundedIcon />} checkedIcon={<CheckCircleRoundedIcon  />} sx={{'&.Mui-checked': {color: '#F8E71C'}}} checked={assessmentStep19Answer == 3} onChange={handleChange} />} label="Sometimes" />
                <FormControlLabel value="4" control={<Radio icon={<RadioButtonUncheckedRoundedIcon />} checkedIcon={<CheckCircleRoundedIcon  />} sx={{'&.Mui-checked': {color: '#F8E71C'}}} checked={assessmentStep19Answer == 4} onChange={handleChange} />} label="Often" />
                <FormControlLabel value="5" control={<Radio icon={<RadioButtonUncheckedRoundedIcon />} checkedIcon={<CheckCircleRoundedIcon  />} sx={{'&.Mui-checked': {color: '#F8E71C'}}} checked={assessmentStep19Answer == 5} onChange={handleChange} />} label="Most of the time" />
              </RadioGroup>
              {
                formError ? 
