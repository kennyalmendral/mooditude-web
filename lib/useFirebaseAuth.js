...
export default function useFirebaseAuth() {
  ...
  const sendPasswordResetEmail = (email) => firebaseAuth.sendPasswordResetEmail(email)

  const verifyPasswordResetCode = (code) => firebaseAuth.verifyPasswordResetCode(code)

  const confirmPasswordReset = (code, password) => firebaseAuth.confirmPasswordReset(code, password)
